@echo off
chcp 65001 >nul
echo Создание триггера TR_BI_AUDITLOG...
cd /d "%~dp0"
...
echo CREATE TRIGGER TR_BI_AUDITLOG FOR AUDITLOG ACTIVE BEFORE INSERT POSITION 0 AS BEGIN IF (NEW.LogID IS NULL) THEN NEW.LogID = GEN_ID(GEN_AUDITLOG_ID, 1); END >> tmp.sql
echo ^;>> tmp.sql
echo SET TERM ;>> tmp.sql
echo COMMIT;>> tmp.sql
