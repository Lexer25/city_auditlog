@echo off
chcp 65001 >nul
echo Создание триггера SS_ACCESSUSER_AUDIT_AD...
cd /d "%~dp0"
...
echo CREATE TRIGGER SS_ACCESSUSER_AUDIT_AD FOR SS_ACCESSUSER >> trigger.sql
echo ACTIVE AFTER DELETE POSITION 0 >> trigger.sql
echo AS >> trigger.sql
...
echo   insert into auditlog (username, tablename, newrecordid, actiondate) values (coalesce(:username, :conn_id), 'ss_accessuser', null, CURRENT_TIMESTAMP); >> trigger.sql
